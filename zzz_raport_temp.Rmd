...
Zauważyć możemy również grupowanie się braków danych. Braki przyjmują wzorzec
wielowymiarowy (brak w więcej niż jednej zmiennej), niemonotonny (braki jedej
zmiennej nie oznaczają braków pozostałych zmiennych) oraz połączony.
...
-   Age - knn / mice
-   Att - mice i rpart (prawie identyczne), wybór mice
-   MOI - knn
...
```{r}
HR_imputowane <- HR %>%
  mutate(Age = ifelse(is.na(HR$Age), im_age_mice, HR$Age)) %>%
  mutate(Attrition = ifelse(is.na(HR$Attrition), im_att_mice, HR$Attrition)) %>%
  mutate(Attrition = ifelse(!is.na(HR$Attrition), HR$Attrition, ifelse(HR$Attrition == 2, "Yes", "No"))) %>%
  mutate(MonthlyIncome = ifelse(is.na(HR$MonthlyIncome), im_moi_knn, HR$MonthlyIncome))
HR_imputowane
```
