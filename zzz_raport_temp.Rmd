...
im_age_knn <- tibble(age1 = hr_knn$Age, age = numeric_HR$Age)
im_att_knn <- hr_knn$Attrition
im_moi_knn <- hr_knn$MonthlyIncome

str(im_age_mice)

aggr(x = c(im_age_knn, numeric_HR$Age), delimiter = )
aggr(x = c(im_att_knn, numeric_HR$Attrition), delimiter = im_att_knn)
aggr(x = c(im_moi_knn, numeric_HR$MonthlyIncome))


# Assuming 'numeric_HR' is the original data and 'hr_knn' is the imputed data
combined_long <- numeric_HR %>%
  as.data.frame() %>%
  pivot_longer(cols = everything(Age, Attrition, MonthlyIncome), names_to = "variable", values_to = "value") %>%
  mutate(type = "Original") %>%
  bind_rows(
    hr_knn %>%
      as.data.frame() %>%
      pivot_longer(cols = everything(age1 = Age, att1 = Attrition, moi1 = MonthlyIncome), names_to = "variable", values_to = "value") %>%
      mutate(type = "Imputed")
  )

ggplot(combined_long, aes(x = value, color = type)) +
  geom_density() +
  facet_wrap(~ variable, scales = "free") +
  labs(title = "Distribution of Original and Imputed Values",
       x = "Value",
       y = "Density") +
  theme_minimal()
